const templateEngine = (function () {
    ...
    const templateEngine = {
        ...
        addContent: function (parent, type, content, data, instruction) {
                ...
                switch(type) {
                    ...
                    case "tag": // create tag
                        return (function () {
                            ...
                            if (content === "[use-data]"){
                                content = data;
                            } else if (typeof(data) === "object" && typeof(data.attributes) === "object") {
                ...
                if (content != undefined && parent != undefined) {
                    ...
                    if (parentElements != undefined && parentElements.length > 0) {
                        ...
                        for (let i = 0; i < parentElements.length; i++) {
                            ...
                            const newElementsWithData = templateEngine.addContent(parentElement, instruction.type, content, data, instruction);
                                ...
                                for (let elementIndex = 0; elementIndex < newElementsWithData.length; elementIndex++) {
                                    ...
                                    if (newElementWithData != undefined) {
                                        ...
                                        if (instructionChildren != undefined && instructionChildren.length > 0) {
                                            ...
                                            let j = 0;
                                            if (instructionChildren[j]) {
                                                do {
                                                    instructionChildren[j].parent = instruction;
                                                    apply(instructionChildren[j], newElement, newElementWithData.data);
                                                    j++;
                                                }
                                                while (instructionChildren[j]);
                                            }
